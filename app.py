...
# ═══════════════════════════════════════════════════════════════════════════════
#  PORTFOLIO PASSWORD CONFIG
#  Set PORTFOLIO_PASSWORD env var to protect the Portfolio tab.
#  Leave empty (or unset) to hide the Portfolio tab entirely (analysis-only mode).
# ═══════════════════════════════════════════════════════════════════════════════
...
#
#  One codebase, two Railway services:
#
#  MASTER (private — your personal app):
#    - Set INTERNAL_API_KEY=<any secret string>
#    - Has persistent Volume at /data, NEPSE_DATA_DIR=/data
#    - Has PORTFOLIO_PASSWORD set
#    - Fetches and owns all broker floorsheet cache
#    - Exposes /api/internal/* for the public app to read
#
#  CLIENT (public — analysis-only app):
#    - Set MASTER_URL=https://your-master.up.railway.app
#    - Set INTERNAL_API_KEY=<same secret string as master>
#    - No Volume needed — reads cache from master over HTTP
#    - Use index_analysis_only.html as static/index.html
#    - All broker/cache requests are proxied to master automatically
#
#  If MASTER_URL is empty → this instance IS the master (fetches directly).
#  If MASTER_URL is set   → this instance IS the client (proxies to master).
# ═══════════════════════════════════════════════════════════════════════════════
INTERNAL_API_KEY = os.environ.get("INTERNAL_API_KEY", "").strip()
MASTER_URL       = os.environ.get("MASTER_URL", "").strip().rstrip("/")
IS_CLIENT        = bool(MASTER_URL)   # True = public/analysis app
IS_MASTER        = not IS_CLIENT      # True = private/portfolio app

import requests as _req_lib

def _key_valid():
    """Check X-Internal-Key header on incoming request."""
...
def _proxy(path, method="GET", **kwargs):
    """
    Forward a request to the master app.
    path  — e.g. "/api/broker/floorsheet"
    Returns (dict, status_code) ready to pass to jresp().
    """
    url = MASTER_URL + path
    headers = {"X-Internal-Key": INTERNAL_API_KEY}
    try:
        if method == "GET":
            r = _req_lib.get(url, params=request.args, headers=headers, timeout=60)
        else:
            r = _req_lib.request(method, url, headers=headers,
                                 json=request.get_json(silent=True), timeout=60)
        return r.json(), r.status_code
    except Exception as e:
        return {"error": f"Master unreachable: {e}"}, 502
...
def api_config():
    """Frontend app config — portfolio mode + master/client mode."""
    enabled   = bool(PORTFOLIO_PASSWORD)
    protected = bool(PORTFOLIO_PASSWORD)
    return jresp({
        "portfolio_enabled":   enabled,
        "portfolio_protected": protected,
        "mode":      "protected" if protected else "disabled",
        "is_client": IS_CLIENT,
        "master_url": MASTER_URL if IS_CLIENT else None,
    })

@app.route("/api/auth/portfolio", methods=["POST"])
def auth_portfolio():
    ...
    if not PORTFOLIO_PASSWORD:
        return jresp({"ok": False, "error": "Portfolio is disabled on this instance."}), 403
    if pw == PORTFOLIO_PASSWORD:
        token = hashlib.sha256((pw + "nepse_tunnel_salt").encode()).hexdigest()[:32]
        return jresp({"ok": True, "token": token})
    return jresp({"ok": False, "error": "Incorrect password."}), 401

# ── Internal API (master exposes these; client calls them) ──────────────────
# Protected by X-Internal-Key header. Only useful when INTERNAL_API_KEY is set.

@app.route("/api/internal/health")
def internal_health():
    """Client pings this to verify master is reachable and key is valid."""
    if not _key_valid():
        return jresp({"ok": False, "error": "Unauthorized"}), 401
    stats = cache_stats()
    return jresp({
        "ok":            True,
        "mode":          "master",
        "cache_dates":   stats.get("total_dates", 0),
        "cache_symbols": stats.get("total_symbols", 0),
    })

@app.route("/api/internal/broker/names")
def internal_broker_names():
    if not _key_valid(): return jresp({"error":"Unauthorized"}), 401
    try:
...
@app.route("/api/internal/broker/floorsheet")
def internal_broker_floorsheet():
    """Pass-through to the regular floorsheet handler — key-gated."""
    if not _key_valid(): return jresp({"error":"Unauthorized"}), 401
    return api_broker_floorsheet()

@app.route("/api/internal/broker/analyze")
def internal_broker_analyze():
    if not _key_valid(): return jresp({"error":"Unauthorized"}), 401
    return api_broker_analyze()

@app.route("/api/internal/broker/accumulation")
def internal_broker_accumulation():
    if not _key_valid(): return jresp({"error":"Unauthorized"}), 401
    return api_broker_accumulation()

@app.route("/api/internal/broker/daily")
def internal_broker_daily():
    if not _key_valid(): return jresp({"error":"Unauthorized"}), 401
    return api_broker_daily()

@app.route("/api/internal/cache/status")
def internal_cache_status():
    if not _key_valid(): return jresp({"error":"Unauthorized"}), 401
    return jresp(cache_stats())

@app.route("/api/internal/cache/update", methods=["POST"])
def internal_cache_update():
    """Allow client to trigger a cache update on master."""
    if not _key_valid(): return jresp({"error":"Unauthorized"}), 401
    return api_cache_update()
...
def api_broker_names():
    if IS_CLIENT:
        data, code = _proxy("/api/internal/broker/names")
        return jresp(data), code
    try:
        names  = get_broker_names()
        result = [{"code": k, "name": v} for k, v in names.items() if "_" not in k]
        result.sort(key=lambda x: x["code"].zfill(5))
...
def api_broker_floorsheet():
    ...
    Client mode: proxied to master app.
    """
    if IS_CLIENT:
        data, code = _proxy("/api/internal/broker/floorsheet")
        return jresp(data), code
...
def api_broker_analyze():
    ...
    if IS_CLIENT:
        data, code = _proxy("/api/internal/broker/analyze")
        return jresp(data), code
    symbol   = request.args.get("symbol","").upper().strip()
    date_str = request.args.get("date", datetime.now().strftime("%Y-%m-%d"))
    if not symbol:
...
def api_broker_accumulation():
    ...
    """
    if IS_CLIENT:
        data, code = _proxy("/api/internal/broker/accumulation")
        return jresp(data), code
...
def api_broker_daily():
    ...
    if IS_CLIENT:
        data, code = _proxy("/api/internal/broker/daily")
        return jresp(data), code
    broker    = request.args.get("broker", "").strip()
    symbol    = request.args.get("symbol", "")
...
def api_cache_status():
    ...
    In client mode, shows master's cache status."""
    if IS_CLIENT:
        data, code = _proxy("/api/internal/cache/status")
        return jresp(data), code
    return jresp(cache_stats())
...
def api_cache_update():
    ...
    In client mode, triggers update on master."""
    if IS_CLIENT:
        data, code = _proxy("/api/internal/cache/update", method="POST")
        return jresp(data), code
    data   = request.get_json(silent=True) or {}
